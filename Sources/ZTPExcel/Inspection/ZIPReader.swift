// ZIPReader.swift
// ZTPExcel – Minimal ZIP archive reader for .xlsx inspection.

import Foundation
import Compression

/// Errors thrown when reading a ZIP archive.
enum ZIPReaderError: Error, Sendable {
    case invalidArchive
...
struct ZIPReader: Sendable {
    ...
    static func readEntries(from data: Data) throws -> [Entry] {
        guard data.count >= 22 else {
            throw ZIPReaderError.dataTooSmall
        }
    ...
    private static func findEOCD(in data: Data) throws -> Int {
        ...
        throw ZIPReaderError.endOfCentralDirectoryNotFound
    }
    ...
    private static func parseEOCD(data: Data, eocdOffset: Int) throws -> (cdOffset: Int, entryCount: Int) {
        ...
        guard cdOffset >= 0 && cdOffset < data.count else {
            throw ZIPReaderError.invalidArchive
        }
    ...
    private static func parseCentralDirectoryEntry(
        ...
        guard offset + 46 <= data.count else {
            throw ZIPReaderError.invalidCentralDirectoryEntry
        }
    ...
    private static func readLocalFile(data: Data, offset: Int) throws -> Data {
        guard offset + 30 <= data.count else {
            throw ZIPReaderError.invalidLocalFileHeader
        }
